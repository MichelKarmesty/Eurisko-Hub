...
* **Admin Ticket Actions (RBAC, [ADR-002](decisions/ADR-002.md) / [ADR-003](decisions/ADR-003.md) / [ADR-010](decisions/ADR-010.md)):** An Admin can **assign** an unclaimed ticket to an agent of the matching department, **cancel** a request softly with a required reason (the ticket and its history are kept), **permanently delete a `Resolved` ticket** once it is finished ([ADR-010](decisions/ADR-010.md)), and change any ticket's status only as an explicit **override** that records a reason. These are the "actions beyond viewing" this spec originally deferred.
