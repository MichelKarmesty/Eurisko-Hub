...
* **Scenario 4: Recovering a Forgotten Password**
  * *Action:* An employee who cannot remember their password clicks **Forgot password?** on the sign-in screen and enters their email; a one-time link is sent to that address and they choose a new password from the link. Alternatively the Admin opens **Users → Reset password** and hands them a one-time link (or, when the Admin is the locked-out person, an operator runs `scripts/reset-password.mjs` offline). A signed-in user instead uses **Change password**.
  * *Result:* The new password works immediately and the old one stops working; the one-time link expires and cannot be reused, the reset form gives the same generic answer whether or not the email is registered (so it cannot be used to discover accounts), the Admin never sees or chooses the password, and no password is ever displayed or stored in readable form.
