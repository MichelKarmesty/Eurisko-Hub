...
> (ADR-004), `db-file-folder.spec.ts` and `mail-smtp.spec.ts` (restored with the
> mail transports by ADR-008) - and the DOM suite gained
> `change-password.ui.test.tsx`. So `npm test` now runs **12 spec files**, and
> `cd e2e && npm run test:ui` **5 DOM tests** (five AI-eval cases skip when no AI
> provider is running); `npm run test:ai-eval` runs the AI evals on their own.
> The Week 3 numbers above are kept as the record of that delivery.
>
> Latest verified run (browser layer skipped): backend
