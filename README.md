...
# option A2 — a SECOND sender, tried when the first one fails (e.g. Outlook backup)
export SMTP_ALT_HOST=smtp-mail.outlook.com
export SMTP_ALT_USER=you@outlook.com
export SMTP_ALT_PASS=your-outlook-app-password
export SMTP_ALT_FROM="Eurisko Hub <you@outlook.com>"   # must be that account's own address
...
One command asks for one or two addresses + App Passwords, writes them into
`backend/.env` and sends a real test message through each sender:
...
| `GET /users` · `POST /users` · `PATCH /users/:id/role` · `PATCH /users/:id/active` · `DELETE /users/:id` | Admin | manage accounts: list (`?includeInactive=true` shows deactivated rows), create, change role, **deactivate / reactivate** (`active: false` revokes the login at once, `true` restores the same account and its tickets), **delete any account** (an account with tickets/history is only deactivated, one with none is really deleted). A deactivated row keeps its email, so re-creating it is refused — reactivate instead. No path can empty the Admin seat: you cannot change/deactivate your own Admin role or account, and the last active Admin cannot be demoted or deleted |
